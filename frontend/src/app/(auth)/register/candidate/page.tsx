...
export default function RegisterCandidatePage(): JSX.Element {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (values: FormValues) => registerCandidate(values.email, values.password),
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Start applying to jobs on ProHire.</CardDescription>
      </CardHeader>
      <CardContent>
        <form id="register-candidate-form" onSubmit={handleSubmit((v) => mutate(v))} className="flex flex-col gap-4">
      ...
      <CardFooter className="flex flex-col gap-3">
        <Button type="submit" form="register-candidate-form" className="w-full" disabled={isPending}>
        ...
        <p className="text-xs text-muted-foreground text-center">
          ...
          <Link href="/register/company" className="text-foreground hover:underline">Register as company</Link>
        </p>
      </CardFooter>
    </Card>
  );
